...
class Api::TipoProcedimientosController < ApplicationController
    ...
    def get_plan_auditoria
        ...
        @plan = PaquetePlan.where(ano:@ano_plan).joins("LEFT JOIN paquete_plan_to_procedimientos ON paquete_plans.id= paquete_plan_to_procedimientos.idpaquete LEFT JOIN  Procedimientos ON Procedimientos.id = paquete_plan_to_procedimientos.idprocedimiento LEFT JOIN Procesos ON Procedimientos.idprocesos = Procesos.Id LEFT JOIN tipo_procesos ON  tipo_procesos.id = Procesos.tipo LEFT JOIN paquetes_plan_to_fechas ON paquetes_plan_to_fechas.idpaquete=paquete_plans.id ").select(" paquete_plans.id as id_paquete, paquete_plans.nombre as nombre_paquete,tipo_procesos.nombre as tipo_proceso,Procesos.nombre as nombre_proceso, Procedimientos.id ,Procedimientos.nombre nombre_procedimiento , paquetes_plan_to_fechas.responsable, fecha_plan_i,   fecha_plan_f , dias ,fecha_inicio,fecha_fin,comentarios ").order("paquetes_plan_to_fechas.fecha_plan_i desc")
